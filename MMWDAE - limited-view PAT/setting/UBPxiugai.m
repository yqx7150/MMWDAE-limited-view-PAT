...
picture1=imread('F:\shujuji\data\image_size_256_1channal\腹部\512\1.png');
picture1=im2double(picture1);
p=picture1;

tot_grid_size = grid_size_SL + 184; %% the extra 100 grid points are used for PML and placing transducers
p0 = zeros(tot_grid_size);

p0(93:tot_grid_size-92,93:tot_grid_size-92) = p;
...
sensor_radius = 21.6e-3; % [m]
sensor_angle = 24/12*pi; % [rad]
sensor_pos = [0, 0]; % [m]
...
% 
% x_ii = cart_sensor_mask(1,ii)/dx;
% y_ii = cart_sensor_mask(2,ii)/dy;
% s1(x_ii,y_ii)=1;
% end
% figure;imshow(s1)
% 
...
figure;imshow(p0_recon0);
    ...
    p0_recon0 = p0_recon./(2*pi);
    b=p0_recon0(93:348,93:348);  
    b_max=max(max(b));
    b_min=min(min(b));
    b_normal=(b-b_min)/(b_max-b_min);
    imshow(b_normal);
%     folder = 'F:\code\gs\result\zzy\1\';
%     folder_out = strcat(folder,'1_171.png');
%     imwrite(b_normal,folder_out);